...
export function BenefitsSection() {
  ...
  return (
    <section className={`${theme.gradients.primary} ${theme.spacing.section} relative overflow-hidden min-h-screen sm:min-h-[80vh] 2xl:min-h-[70vh]`}>
      ...
      <motion.div
        ...
        className="mb-6 sm:mb-8 2xl:mb-12 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-8 max-w-7xl mx-auto px-4"
      >
        {[
          ...
          <motion.div
            ...
            className={`${theme.cards.base} ${theme.cards.variants.gradient} ${theme.shadows.card} 
              p-6 sm:p-8 text-center hover:bg-gradient-to-br hover:from-[#60A5FA]/5 hover:to-transparent rounded-2xl`}
          >
            <div className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-[#60A5FA] to-[#F472B6] bg-clip-text text-transparent mb-2">
              {stat.number}
            </div>
      ...
      <div className={`${theme.spacing.container} max-w-[1920px] 2xl:max-w-[1600px] mx-auto px-4 sm:px-6`}>
        {/* Section Header */}
        <motion.div
          ...
          className="text-center mb-8 sm:mb-12 2xl:mb-16"
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl 2xl:text-6xl font-bold mb-3 sm:mb-4 gradient-text">
            Why Choose Greetly?
          </h2>
          <p className="text-base sm:text-lg 2xl:text-xl text-text-secondary max-w-2xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row items-start justify-between gap-8 max-w-6xl mx-auto">
          ...
          <div className="w-full lg:w-1/2 relative">
            <div className="relative mx-auto w-full sm:w-[500px] md:w-[600px] h-[400px] sm:h-[500px] md:h-[600px]">
              <div className={`absolute inset-0 ${theme.cards.base} ${theme.cards.variants.gradient} ${theme.shadows.card} rounded-[30px] sm:rounded-[40px] border-4 sm:border-8 border-gray-200 overflow-hidden`}>
                <div className="absolute top-0 left-0 right-0 h-8 sm:h-12 w-full bg-gray-100 rounded-t-[24px] sm:rounded-t-[32px] flex items-center justify-center">
                  <div className="w-16 sm:w-24 h-4 sm:h-6 bg-gray-200 rounded-full"></div>
                </div>
              ...
              <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 w-16 sm:w-24 h-1 sm:h-2 bg-gray-200 rounded-full"></div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-1/2 space-y-4 sm:space-y-6">
            {benefits.map((benefit, index) => (
              ...
              >
                <div className={`${theme.cards.base} ${theme.cards.variants.gradient} ${theme.shadows.card} p-4 sm:p-6 rounded-2xl
                  ${activeBenefit === index ? 'border-2 border-[#60A5FA]' : ''}`}
                >
                  <div className="flex items-start gap-3 sm:gap-4 rounded-2xl">
                    <div className={`p-2 sm:p-3 rounded-xl text-white bg-gradient-to-br ${benefit.gradient}`}>
                      {benefit.icon}
                    </div>
                    <div>
